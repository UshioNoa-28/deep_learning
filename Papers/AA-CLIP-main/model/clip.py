...
def _rescan_model_configs():
    ...
    # 扫描 model_configs/*.json，建立 model_name -> 配置字典 的注册表
...
def load_state_dict(checkpoint_path: str, map_location='cpu'):
    # 兼容不同 checkpoint 格式，统一拿到纯 state_dict
...
def create_model(
    ...
    # 这是 train/test 里最常调用的入口：构建 CLIP 并按需加载预训练权重
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        # 读取OpenAi的CLIP模型
        state_dict = model_pre.state_dict()# 拿到这个模型的权重
        if output_dict and hasattr(model_pre, "output_dict"):
            model_pre.output_dict = True
        model = CLIP(**model_cfg, cast_dtype=cast_dtype)# 用读取到的配置文件New一个本地的CLIP模型
    ...
    return model
# 读取OpenAi的CLIP模型权重，然后加载到本地的CLIP模型中
