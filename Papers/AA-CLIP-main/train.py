...
def train_text_adapter(
    ...
    # Stage 1: 训练文本适配器
    # 目标：
    # 1) 让“正常/异常”文本锚点与图像 patch 更可分
    # 2) 仅更新 model.text_adapter，不更新图像编码器
    # =========================
    for epoch in range(start_epoch, text_epoch):
        ...
        for input_data in tqdm(train_loader):
            image = input_data["image"].to(device) # 图像
            mask = input_data["mask"].to(device)   # mask 即为异常位置检测的结果
            ...
            # 说明：
            # - 最后一维 2 表示 [normal, abnormal] 两个文本锚点
            # - 每个样本按自己的 class_name 取对应文本锚点
            # forward image
            with torch.no_grad():
                # 注意：stage1 不训练图像分支，因此图像特征都在 no_grad 下提取
                ...
                # 这里把全局 cls_token 加回每个 patch，增强 patch 的类别语义信息
            ...
            for f in patch_features:
                ...
                # orthogonal_loss:
                # 希望 normal/abnormal 文本方向尽量正交，避免语义混叠
                orthogonal_loss = (
        ...
        torch.save(
            {
                "epoch": epoch + 1,
                # stage1 只需要保存 text_adapter + text_optimizer
...
def train_image_adapter(
    ...
    for epoch in range(start_epoch, image_epoch):
        ...
        for input_data in tqdm(train_loader):
            ...
            # epoch_text_feature: [B, 768, 2]，每个样本按类别拿自己的 normal/abnormal 锚点

            # forward image
            patch_features, det_feature = model(image)
            # patch_features: list[[B, N_patch, 768]]
            # det_feature: [B, 768]
            # calculate similarity and get prediction
            loss = 0.0
            det_feature = det_feature.unsqueeze(1)
            cls_preds = torch.matmul(det_feature, epoch_text_feature)[:, 0]
            # cls_preds: [B, 2]，分别对应 normal/abnormal 相似度
...
def main():
    ...
    clip_surgery.eval()
    # clip_surgery 仅用于 stage1 提取“手术后的 patch 特征”，不参与训练
    ...
    model.eval()
    # 这里调用 eval() 不是“禁止训练”，而是保持 CLIP 主体统计行为稳定；
    # 真正可训练的是 adapter 参数，它们由 optimizer 显式更新
    # set optimizer
    ...
    # 这里分两个 dataloader：
    # - text_dataloader: stage1 用
    # - image_dataloader: stage2 用
